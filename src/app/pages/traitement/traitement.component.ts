...
@Component({
  ...
  styleUrls: ['./traitement.component.scss','./material.css']
})
...
//********************************************get USERS form PM******************************************
  ...
  ngOnInit() {
    ...
    //       }
    //       for(var j=1;j<3;j++){
    //         let elementbutton=document.getElementById("buttonid"+j);
    //         elementbutton.className= "btn btn-primary btn-rounded w-200p mb-1 mr-1 butttons-style "+sessionStorage.getItem("skin");
    //         elementbutton.className="btn btn-info btn-sm butttons-style "+sessionStorage.getItem("skin");
    //       }
    //
    //     }
    //     else
  ...
  onToolbarPreparing(e) {
    e.toolbarOptions.items.unshift({
      ...
      options: {
        ...
        onClick: this.ajouterAnnotation.bind(this)
      }
    });
  }
  ...
  routerdocument($event,dataform) {

//*******router document and mis a jour in db *********************

    let document = this.updateDocument(dataform);
    let resultNextTask = $event;
    //console.log("listeAffecter",this.MYJSONActeur.listAffecter,"listeDiffusion",this.MYJSONActeur.listeDiffusion,"liste responsable des taches",this.MYJSONActeur.listActeur)

    /* this.flowable.nextTask(document.activityid, decision,this.ArrayToString(this.MYJSONActeur.listAffecter),this.ArrayToString(this.MYJSONActeur.listeDiffusion)+this.ArrayToString(this.MYJSONActeur.listActeur), this.courrierArrive.comment, this.courrierArrive.categorie, localStorage.getItem('profileUser')).subscribe(
         resultNextTask => {

             localStorage.setItem('taskId', resultNextTask[0]);
             console.log('result next task********', resultNextTask);
             let document = this.updateDocument(dataform);
             document.activityName = resultNextTask[4];
             document.activityid = resultNextTask[0];
             document.responsable=resultNextTask[3];
             this.servicearriver.onAddCourrierArriver(document, resultNextTask[1], resultNextTask[2], 1).subscribe(data => {
                 console.log('save', data);

                 this.servicearriver.courrierArrive = data;
             });
             this.route.navigate(['/courriers']);
         });*/
