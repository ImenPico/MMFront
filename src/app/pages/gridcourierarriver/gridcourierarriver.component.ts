...
@Component({
  selector: 'app-gridcourierarriver',
  templateUrl: './gridcourierarriver.component.html',
  styleUrls: ['./gridcourierarriver.component.scss']
})

export class GridcourierarriverComponent implements OnInit {
    ...
    this.alfrescoApi.getNodeChilds(nodeRef).then((data1) => {
      ...
      for (let i = 0; i < data1.list.entries.length; i++) {
        if (data1.list.entries[i].entry.name) {
          ...
          // );
          this.Filealfresco.push(fileAlfresco)

        }

      }
      console.log("fils======================>", this.Filealfresco)

      return this.Filealfresco
